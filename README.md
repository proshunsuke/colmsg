...
* 保存されるメッセージは次のディレクトリ構造で保存されます
  * ```shell script
    ...
    ```
* ファイル名の形式は `<シーケンス番号>_<種類>_<日付>_<投稿者名>.<拡張子>` となっています
  * シーケンス番号はメッセージの時系列を表す番号になっています。投稿者名の前までの並び順を維持しているため、ファイルブラウザで辞書順に並べると保存したメッセージが時系列通りに並びます
  * 投稿者を特定できない場合は `unknown` が使われます
  * 種類の数字は以下のように対応しています
    * 0: テキストメッセージ
    * 1: 画像
    * 2: 動画
