...
impl Client {
    ...
    pub fn get_request<RT>(&self, path: &str, access_token: &str, parameters: Option<Vec<(&str, &str)>>) -> Result<RT>
        ...
        let url = Url::parse(&self.base_url).unwrap().join(&path).unwrap();
        let url = Url::parse_with_params(url.as_str(), &iter).unwrap();
        ...
        match result {
            ...
            Err(e) => {
                let error_message = format!(
                    "error: {}, request url: {}, response body: {}", e.to_string(), &url_str, &body
                );
                Err(error_message.into())
            }
        }
    ...
    fn insert_headers(&self, mut header: HeaderMap) -> HeaderMap {
        header.insert(ACCEPT, "application/json".parse().unwrap());
        header.insert(CONTENT_TYPE, "application/json".parse().unwrap());
        header.insert("X-Talk-App-ID", "jp.co.sonymusic.communication.keyakizaka 2.0".parse().unwrap());
        header.insert(ACCEPT_LANGUAGE, "ja-JP".parse().unwrap());
        header.insert(USER_AGENT, "Dalvik/2.1.0 (Linux; U; Android 6.0; Samsung Galaxy S7 for keyaki messages Build/MRA58K)".parse().unwrap());
        header.insert(CONNECTION, "Keep-Alive".parse().unwrap());
        header.insert(ACCEPT_ENCODING, "gzip".parse().unwrap());
        header.insert(TE, "gzip, deflate; q=0.5".parse().unwrap());
        header
    }

    fn insert_optional_headers(&self, mut header: HeaderMap, access_token: &str) -> HeaderMap {
    ...
    #[cfg(feature = "401")]
    fn insert_401_header(&self, mut header: HeaderMap) -> HeaderMap {
        if rand::thread_rng().gen_range(0, 6) == 0 {
            header.insert("Prefer", "code=401".parse().unwrap());
        };
        header
    }

    #[cfg(not(feature = "401"))]
    fn insert_401_header(&self, header: HeaderMap) -> HeaderMap { header }
}
